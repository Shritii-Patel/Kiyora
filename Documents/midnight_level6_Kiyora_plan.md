# 🔐 Kiyora — Private Access. Verifiable Trust. Zero Identity Exposure.

## Idea Overview

Kiyora is a privacy-preserving credential and access verification protocol built on the Midnight Network Preprod environment.
...
Kiyora uses Midnight’s Zero-Knowledge capabilities to verify eligibility privately, while also supporting real access requirements such as credential expiration, revocation, multiple issuers, role-based permissions, and protection against credential or proof reuse.
...
Kiyora solves this by separating identity from eligibility.
...
Kiyora is a reusable Zero-Knowledge Access Gateway.
...
Kiyora is designed as reusable infrastructure, not just a single dApp.

A lightweight Kiyora SDK/API will allow Midnight developers to integrate private access verification into their own applications.
...
This makes Kiyora useful as a privacy layer that other Midnight applications can build on instead of implementing their own credential verification system from scratch.

## Real Sample dApp

To demonstrate the complete integration, Kiyora will include a sample Midnight dApp with a protected private resource.
...
Credential Issuer → Private Credential → Kiyora SDK → ZK Verification → Compact Contract → Protected Feature Unlocked
...
This demonstrates Kiyora protecting a real application feature rather than only showing a standalone proof demo.

## Why Midnight?

Privacy is the foundation of Kiyora.

Midnight allows Kiyora to combine private inputs, Zero-Knowledge proofs, programmable access policies, and verifiable blockchain execution.
...
Kiyora will run on Midnight Preprod with an end-to-end flow:
...
The sample dApp will use the Kiyora SDK/API and deployed Compact contract to demonstrate this complete flow on Preprod.

## 🚀 Vision

Kiyora aims to become a reusable privacy and access infrastructure layer for Midnight applications.
...
Kiyora turns those requirements into privacy-preserving, programmable access rules that can be integrated into real applications.

Verify what matters. Keep everything else private.

Kiyora — Verifiable by design. Private by default.
