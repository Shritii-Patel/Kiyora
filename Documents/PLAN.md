...
Kiyora is a privacy-preserving credential and access verification system built on Midnight.
...
If those conditions are satisfied, Kiyora returns a valid access result without exposing unnecessary private information.
...
Kiyora uses Zero Knowledge proofs to answer that question while keeping the underlying credential information private.

---

## 3. Core System

Kiyora will have four main parts:
...
Kiyora will support **multiple trusted issuers**, allowing an application to accept credentials from different approved organizations.
...
An external application integrates Kiyora and asks for verification before unlocking a protected feature or resource.
...
Kiyora will therefore support a basic credential lifecycle.
...
Kiyora verifies that the credential is still within its allowed validity period before granting access.
...
Kiyora checks the revocation state during access verification.
...
Kiyora will not depend on one central credential issuer.
...
This makes Kiyora useful across different organizations and applications.
...
This allows one Kiyora-based credential system to protect multiple resources with different permissions.

---

## 7. Time-Based Access

Kiyora will also support policies where access is valid only during a specific period.
...
Preventing reuse is an important part of the Kiyora design.

A valid credential should not automatically mean that the same proof can be copied and replayed.

Kiyora will use **nullifier-based protection** where required.
...
Kiyora is planned as reusable infrastructure rather than only one standalone application.

A developer should be able to integrate Kiyora into another Midnight application without rebuilding the complete credential verification flow.

The project will therefore include an initial **Kiyora TypeScript SDK/API**.

Example integration:

```ts
const result = await Kiyora.verifyAccess({
...
The SDK will provide a simple interface around the Kiyora verification flow.

Its responsibilities will include:

* requesting access verification,
* interacting with Kiyora contracts,
...
It only needs to demonstrate that Kiyora can be integrated into another application as an access-verification layer.

---

## 13. Sample Midnight dApp

A separate sample application will demonstrate a complete Kiyora integration.

### Kiyora Developer Portal
...
User Opens Developer Portal
      ↓
Portal Calls Kiyora SDK
      ↓
Kiyora Requests Private Verification
      ↓
...
This sample application is important because it demonstrates that Kiyora is not only an isolated credential demo.

It shows another Midnight application actually using Kiyora as infrastructure.
...
Verify your eligibility with Kiyora.
```
...
| Phase 5 | Frontend integration         | Complete Kiyora user flow      |
| Phase 6 | Kiyora SDK                  | Reusable TypeScript integration |
...
* A basic Kiyora TypeScript SDK/API.
...
Kiyora starts with credential-based private access, but the goal is to make that capability reusable.

Instead of every Midnight developer building their own credential verification and privacy logic, an application should be able to integrate Kiyora and ask:

> **“Does this user satisfy my access policy?”**

Kiyora verifies that privately and returns the minimum result the application needs.

The user keeps their sensitive credential information private.

The application still gets verifiable access control.

That is the core purpose of Kiyora.
