# Product Proposal — Kiyora

**Program:** New Moon to Full: Monthly Moonshots on Midnight
**Project:** Kiyora — Private Access & Credential Verification
...
Kiyora is designed to make that verification private.

## Product

**Kiyora** is a reusable private access and credential verification layer built on Midnight.
...
Kiyora is intended to become infrastructure that other Midnight developers can reuse instead of rebuilding private access logic for every application.

The MVP will provide a simple **Kiyora SDK/API** for developers to:
...
The SDK will handle the Kiyora verification flow, including interacting with the credential/access contract and returning the verification result to the integrating application.

This allows another dApp to use Kiyora as a private access layer without implementing its own credential verification system from scratch.

## Sample dApp — Kiyora Developer Portal

To demonstrate that Kiyora works as reusable infrastructure, I will build a small Midnight dApp that integrates the Kiyora SDK.
...
**Issuer creates credential → User receives credential → User opens protected dApp → dApp requests Kiyora verification → Zero Knowledge proof verifies eligibility → Kiyora returns verification result → Protected feature unlocks**
...
This sample dApp will demonstrate the full integration rather than showing Kiyora only as an isolated proof-of-concept.

## Why Midnight

Kiyora needs users to prove facts about private credentials without exposing the credentials themselves.
...
With Kiyora on Midnight, the application can verify:
...
That privacy boundary is the main reason Kiyora is being built on Midnight.
...
6. Build the Kiyora SDK/API for external dApp integration.
7. Build a sample Midnight Developer Portal protected using the Kiyora SDK.
...
Kiyora should not be only a single credential-gated application.
